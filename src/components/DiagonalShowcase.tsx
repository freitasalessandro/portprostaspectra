      ...
      <div className="relative -mx-12 rotate-[-2deg]">
        ...
        >
          {[...cases, ...cases].map((item, i) => (
            <CaseCard key={`r1-${i}`} item={item} showMetric />
          ))}
        </motion.div>
        ...
        >
          {[...cases.slice(2), ...cases.slice(0, 2), ...cases].map((item, i) => (
            <CaseCard key={`r2-${i}`} item={item} showStatus />
          ))}
        </motion.div>
      </div>
    </section>
...
const CaseCard = ({
  ...
  <motion.div
    ...
    className="group flex-shrink-0 w-[320px] md:w-[440px] h-[200px] md:h-[240px] rounded-2xl border border-border/30 bg-card/20 backdrop-blur-sm hover:border-primary/50 transition-all duration-500 p-6 md:p-8 flex flex-col justify-between cursor-default relative overflow-hidden"
  >
